...
export function LoginForm() {
  ...
  return (
    <form
      ...
      className="w-full max-w-sm rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-7 shadow-sm"
    >
      <h1 className="mb-1 text-2xl font-semibold">Bem-vindo de volta</h1>
      <p className="mb-6 text-sm text-[var(--color-text-muted)]">
        Entre para acessar seu tenant financeiro.
      </p>
      ...
      <p className="mt-4 text-center text-sm text-[var(--color-text-muted)]">
        Novo por aqui?{' '}
        <Link href={routes.register} className="text-[var(--color-primary)]">
          Registre-se
        </Link>
      </p>
    </form>
